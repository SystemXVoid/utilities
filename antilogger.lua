...
local place = "Unknown"
local whitelistautoconfig = true -- wouldn't recommend setting this to false, this will keep you safer.
...
local waitTime = tick() + 2
if hookfunction == nil or hookmetamethod == nil then repeat task.wait() until (hookfunction and hookmetamethod) or waitTime <= tick() end 
...
task.spawn(function()
    if not whitelistautoconfig then 
        return 
    end
    repeat
