...
local function CreateMiniToggle(Parent, Name, DefaultValue, LayoutOrder, CallBack)
	...
	UITextSizeConstraint.Parent = MiniToggle
	UITextSizeConstraint.MaxTextSize = 13
...
local SaveFileName = "GodSploitPrivateSaving.lua"
...
local function CreateSettingsFile()
	...
	Settings = {
		...
		KillAura = {Range = 20, CustomAnimation = true, ToolCheck = false, KeyBind = "..."},
		...
		Strafe = {Range = 18, GoBackwards = true, KeyBind = "..."},
		Speed = {Speed = 23, HeatSeeker = true, KeyBind = "..."},
		Reach = {KeyBind = "..."},
...
local function SwordHit(Entity, Weapon, NearestEntityDistance)
	...
	local SelfPosition = LocalPlayerHumanoidRootPart.Position + (20 > 14 and Magnitude > 14.4 and (CFrame.lookAt(LocalPlayerHumanoidRootPart.Position, EntityPrimaryPart.Position).lookVector * (Magnitude - 14)) or Vector3.zero)
	...
	task.spawn(function()
		if Settings.Strafe.Value == true and NearestEntityDistance <= Settings.Strafe.Range then		
			task.spawn(function()
				if Settings.Strafe.GoBackwards == true then
					local TweenInformation = TweenInfo.new((Magnitude / 22.5), Enum.EasingStyle.Linear, Enum.EasingDirection.In, 0, false, 0)
					local StrafeTween = TweenService:Create(LocalPlayerHumanoidRootPart, TweenInformation, {CFrame = CFrame.new(EntityPrimaryPart.Position - (EntityPrimaryPart.CFrame.LookVector * 5))})
			...
			task.spawn(function()
				if Settings.Strafe.GoBackwards == false then
					local TweenInformation = TweenInfo.new((Magnitude / 22.5), Enum.EasingStyle.Linear, Enum.EasingDirection.In, 0, false, 0)
					local StrafeTween = TweenService:Create(LocalPlayerHumanoidRootPart, TweenInformation, {CFrame = EntityPrimaryPart.CFrame})
...
task.spawn(function()
	repeat
		...
		task.spawn(function()		
			if Settings.Aimbot.Value == true and IsAlive(LocalPlayer) then	
				...
				if NearestPlayer and (AntiVoidPart and NearestPlayer.Character.PrimaryPart.Position.Y >= AntiVoidPart.Position.Y or true) and not NearestPlayer.Character:FindFirstChildOfClass("ForceField") and NearestPlayerDistance > Settings.KillAura.Range then
					...
					task.spawn(function()
						if HasItem("fireball") and not HasItem("arrow") then
						...
						if HasItem("snowball") and not HasItem("arrow")then
		...
		task.spawn(function()
			if Settings.Cape.Value == true and IsAlive(LocalPlayer) and IsMoving() and LocalPlayer.Character:FindFirstChild("Cape") and LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Motor") then
			...
			if Settings.Cape.Value == true and IsAlive(LocalPlayer) and not IsMoving() and LocalPlayer.Character:FindFirstChild("Cape") and LocalPlayer.Character:FindFirstChild("Cape"):FindFirstChild("Motor") then
				local TweenInformation = TweenInfo.new(0.15, Enum.EasingStyle.Linear, Enum.EasingDirection.In, 0, false, 0)
...
task.spawn(function()
	...
	RunService.Heartbeat:Connect(function(Delta)
		...
		task.spawn(function()
			if Settings.Speed.Value == true and Settings.Speed.HeatSeeker == false and IsAlive(LocalPlayer) then			
		...
		task.spawn(function()
			if Settings.Speed.Value == true and Settings.Speed.HeatSeeker == true and IsAlive(LocalPlayer) then			
				local SpeedValue = (HeatSeekerValue and (GetSpeed() + 11.13) or (GetSpeed() + 0.333))
				local SpeedCFrame = LocalPlayer.Character.Humanoid.MoveDirection * SpeedValue * Delta
				...
				if (tick() - HeatSeekerTick) >= 0.2 then
					HeatSeekerValue = not HeatSeekerValue
					HeatSeekerTick = tick()
				end
			end
		end)		
...
task.spawn(function()
	repeat
		...
		task.spawn(function()
			if Settings.AutoBuy.Value == true and IsAlive(LocalPlayer) and GetMatchState() ~= 0 and not GetQueueType():find("Skywars") then
				AutoBuy()
			end			
		end)
	until not game
...
task.spawn(function()
	...
	task.spawn(function()
		...
		DropDownButton.Activated:Connect(function()
			...
			if Value == true then
				InstanceUI = CreateMiniToggle(BlatantTab, "HeatSeeker(Beta)", Settings.Speed.HeatSeeker, LayoutOrder + 2, function(CallBack)
					Settings.Speed.HeatSeeker = CallBack
				end)
			end
	...
	task.spawn(function()
		...
		DropDownButton.Activated:Connect(function()
			...
			if Value == true then
				InstanceUI = CreateSlider(BlatantTab, "Speed", Settings.Speed.Speed, 23, LayoutOrder + 3, function(CallBack)
...
task.spawn(function()
	...
	task.spawn(function()
		...
		DropDownButton.Activated:Connect(function()
			...
			if Value == true then
				InstanceUI = CreateSlider(BlatantTab, "Range", Settings.Strafe.Range, 18, LayoutOrder + 3, function(CallBack)
			...
			if Settings.NoNameTag.Value == true then
				if IsAlive(LocalPlayer) and LocalPlayer.Character.Head:FindFirstChild("Nametag") then
					...
					if ReplicatedStorage:FindFirstChild("Nametag") and LocalPlayer.Character.Head:FindFirstChild("Nametag") then
						ReplicatedStorage:FindFirstChild("Nametag"):Destroy()
					end
...
task.spawn(function()
	local GalaxySky, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "GalaxySky", Settings.GalaxySky.Value, function(CallBack)
		...
		if LightingService:FindFirstChild("WinterSky") and Settings.GalaxySky.Value == true then
		...
		if Settings.GalaxySky.Value == false then
			if ReplicatedStorage:FindFirstChild("Sky") and LightingService:FindFirstChild("GalaxySky") and Settings.GalaxySky.Value == false then
...
task.spawn(function()
	local WinterSky, DropDownButton, LayoutOrder, UIGradient = CreateToggle(WorldTab, "WinterSky", Settings.WinterSky.Value, function(CallBack)
		...
		if Settings.WinterSky.Value == true then
			if LightingService:FindFirstChild("Sky") and Settings.WinterSky.Value == true then
				LightingService.Sky.Parent = ReplicatedStorage
			...
			if LightingService:FindFirstChild("GalaxySky") and Settings.WinterSky.Value == true then
				LightingService.GalaxySky:Destroy()
		...
		if Settings.WinterSky.Value == false then
			if ReplicatedStorage:FindFirstChild("Sky") and LightingService:FindFirstChild("WinterSky") and Settings.WinterSky.Value == false then
				LightingService.WinterSky:Destroy()
				...
				InstanceUI = CreateMiniToggle(WorldTab, "Sigma", Settings.Cape.Sigma, LayoutOrder + 2, function(CallBack)
					...
					task.spawn(function()
						if Settings.Cape.Sigma == true then
							task.spawn(function()
								if IsAlive(LocalPlayer) and Settings.Cape.Value == true and LocalPlayer.Character:FindFirstChild("Cape") then
